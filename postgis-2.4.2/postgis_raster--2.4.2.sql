...
CREATE OR REPLACE FUNCTION postgis_raster_lib_version()
    ...
    LANGUAGE 'c' NOT FENCED IMMUTABLE ; -- a new lib will require a new session

CREATE OR REPLACE FUNCTION postgis_raster_scripts_installed() RETURNS text
       AS $$ SELECT '2.4.2'::text || ' r2.1.1' ::text AS version $$
...
CREATE OR REPLACE FUNCTION st_summary(rast raster)
	...
	BEGIN
		...
		metadata := ST_Metadata(rast, upperleftx, upperlefty, width, height, scalex, scaley, skewx, skewy, srid, numbands);
...
CREATE OR REPLACE FUNCTION _ST_MapAlgebraExpr(
	...
	BEGIN
		...
		ELSE
			IF agg.refraster IS NULL THEN
				m := ST_Metadata(rast, upperleftx, upperlefty, width, height, scalex, scaley, skewx, skewy, srid, numbands);
...
CREATE OR REPLACE FUNCTION st_iscoveragetile(rast raster, coverage raster, tilewidth integer, tileheight integer)
	...
	BEGIN
		...
		_rastmeta := ST_Metadata(rast, upperleftx, upperlefty, width, height, scalex, scaley, skewx, skewy, srid, numbands);
		_covmeta := ST_Metadata(coverage, upperleftx, upperlefty, width, height, scalex, scaley, skewx, skewy, srid, numbands);
		...
		tile[0] := floor(cr.columnx / tilewidth) + 1;
		tile[1] := floor(cr.rowy / tileheight) + 1;
	...
	BEGIN
		...
		CASE
			WHEN _returnband = 'BAND1' THEN
				rtn := _ST_MapAlgebraExpr(rast1, band1, rast2, band2, '[rast1.val]', ST_BandPixelType(rast1, band1), 'INTERSECTION', newnodata1::text, newnodata1::text, newnodata1);
				rtn := ST_SetBandNodataValue(rtn, 1, newnodata1);
			WHEN _returnband = 'BAND2' THEN
				rtn := _ST_MapAlgebraExpr(rast1, band1, rast2, band2, '[rast2.val]', ST_BandPixelType(rast2, band2), 'INTERSECTION', newnodata2::text, newnodata2::text, newnodata2);
				rtn := ST_SetBandNodataValue(rtn, 1, newnodata2);
			WHEN _returnband = 'BOTH' THEN
				rtn := _ST_MapAlgebraExpr(rast1, band1, rast2, band2, '[rast1.val]', ST_BandPixelType(rast1, band1), 'INTERSECTION', newnodata1::text, newnodata1::text, newnodata1);
				rtn := ST_SetBandNodataValue(rtn, 1, newnodata1);
				rtn := ST_AddBand(rtn, _ST_MapAlgebraExpr(rast1, band1, rast2, band2, '[rast2.val]', ST_BandPixelType(rast2, band2), 'INTERSECTION', newnodata2::text, newnodata2::text, newnodata2));
...
-- if it is a comma separated list of two numbers then need to use round
   SELECT CASE WHEN split_part(c.val,',', 2) > 0
...
CREATE OR REPLACE FUNCTION _drop_raster_constraint_spatially_unique(rastschema name, rasttable name, rastcolumn name)
	...
	BEGIN
		...
		IF num = 0 THEN
			RAISE NOTICE 'The constraint is null.  Skipping';
			RETURN FALSE;
		END IF;	
	...
	BEGIN
		...
		IF ( schema IS NULL ) THEN
			sql := 'SELECT count(*) '
					...
					EXECUTE sql INTO schema_num;

			IF schema_num > 1 THEN
				RAISE EXCEPTION 'More than one table with name of %, please use function of AddRasterConstraints with schema parameter instead', quote_literal($2);
			END IF;
			...
			BEGIN
				CASE
					...
					WHEN kw IN ('regular_blocking', 'regularblocking') THEN
						RAISE NOTICE 'Unsupport adding coverage tile constraint required for regular blocking';
						rtn := false;
...
CREATE OR REPLACE FUNCTION DropRasterConstraints (
	...
	BEGIN
		...
		IF schema IS NULL THEN
			...
			IF schema_num > 1 THEN
				RAISE EXCEPTION 'More than one table with name of %, please use function of DropRasterConstraints with schema parameter instead', quote_literal($2);
			END IF;
	...
	BEGIN
		FOR x IN 1..2 LOOP
			...
			IF s = ' ' THEN
				...
				IF schema_num > 1 THEN
					RAISE EXCEPTION 'More than one table with name of %, please use function of AddOverviewConstraints with schema parameter instead', quote_literal($2);
				END IF;
...
CREATE OR REPLACE FUNCTION DropOverviewConstraints (
	...
	BEGIN
		...
		IF schema IS NULL THEN
			...
			IF schema_num > 1 THEN
				RAISE EXCEPTION 'More than one table with name of %, please use function of DropOverviewConstraints with schema parameter instead', quote_literal($2);
			END IF;	
...
CREATE OR REPLACE FUNCTION _UpdateRasterSRID(
	...
	BEGIN
		...
		IF schema IS NULL THEN
			...
			IF schema_num > 1 THEN
				RAISE EXCEPTION 'More than one table with name of %, please use function of UpdateRasterSRID with schema parameter instead', quote_literal($2);
			END IF;
