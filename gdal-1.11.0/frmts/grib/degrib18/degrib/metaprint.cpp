...
#endif
#endif
 static THR_LOCAL char *bufferPrint = NULL;
char *Print(const char *label, const char *varName, Prt_TYPE fmt, ...)
{
    /* Copy of message generated so far. */
   ...
   switch (fmt) {
      case Prt_D:
         ...
         reallocSprintf (&bufferPrint, "%s | %s | %ld\n", label, varName, lival);
         break;
      case Prt_DS:
         lival = va_arg (ap, sInt4);
         sval = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %ld (%s)\n", label, varName,
                         lival, sval);
         break;
      case Prt_DSS:
         lival = va_arg (ap, sInt4);
         sval = va_arg (ap, char *);
         unit = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %ld (%s [%s])\n", label,
                         varName, lival, sval, unit);
         break;
      case Prt_S:
         sval = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %s\n", label, varName, sval);
         break;
      case Prt_SS:
         sval = va_arg (ap, char *);
         unit = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %s (%s)\n", label, varName,
                         sval, unit);
         break;
      case Prt_F:
         dval = va_arg (ap, double);
         reallocSprintf (&bufferPrint, "%s | %s | %f\n", label, varName, dval);
         break;
      case Prt_E:
         dval = va_arg (ap, double);
         reallocSprintf (&bufferPrint, "%s | %s | %e\n", label, varName, dval);
         break;
      case Prt_G:
         dval = va_arg (ap, double);
         reallocSprintf (&bufferPrint, "%s | %s | %g\n", label, varName, dval);
         break;
      case Prt_FS:
         dval = va_arg (ap, double);
         unit = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %f (%s)\n", label, varName,
                         dval, unit);
         break;
      case Prt_ES:
         dval = va_arg (ap, double);
         unit = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %e (%s)\n", label, varName,
                         dval, unit);
         break;
      case Prt_GS:
         dval = va_arg (ap, double);
         unit = va_arg (ap, char *);
         reallocSprintf (&bufferPrint, "%s | %s | %g (%s)\n", label, varName,
                         dval, unit);
         break;
      default:
         reallocSprintf (&bufferPrint, "ERROR: Invalid Print option '%d'\n", fmt);
   }
